...
class PhraseTrigger(Trigger):
    def __init__(self, phrase):
        for x in string.punctuation:
            if x in phrase:
                raise Exception('{} is invalid phrase'.format(phrase))
        assert '  ' not in phrase, '{} includes mulitple spaces'.format(phrase)
        self.phrase = phrase.lower()

    def is_phrase_in(self, text):
        word_list = re.split('[ {}]+'.format(string.punctuation), text.lower())
        word_text = ' '.join(word_list)
        findall = [x.start() for x in re.finditer('(?={})'.format(self.phrase), word_text)]
        for index in findall:
            if self.match(word_text[index:]):
                return True
        return False
